...
$nanocli = new php4nano\NanoCLI($C['nano']['node_file']);

// Node call

if ($C['nano']['connection'] == 'rpc') {
    $nanocall = new php4nano\NanoRPCExt($C['nano']['rpc']['host'], $C['nano']['rpc']['port']);
}
...
// #
// ## Configuration if sending amount
// #
...
// #
    ...
    switch ($command) {
        ...
        case 'delegators':
        {
            ...
            foreach ($delegators['delegators'] as $delegator => $balance) {
                if (isset($arguments['balance_min'])) {
                    if (gmp_cmp($balance, $balance_min) < 0) {
                        continue;
                    }
                }
                
                if (isset($arguments['balance_max'])) {
                    if (gmp_cmp($balance, $balance_max) > 0) {
                        continue;
                    }
                }
                ...
                if (isset($arguments['percent_limit'])) {
                    if ($delegators_array[$delegator]['percent_cumulative'] >= $percent_limit) {
                        break;
                    }
                }
            }
        ...
        case 'representatives':
        {
            ...
            foreach ($representatives['representatives'] as $representative => $weight) {
                if (isset($arguments['weight_min'])) {
                    if (gmp_cmp($weight, $weight_min) < 0) {
                        continue;
                    }
                }
                
                if (isset($arguments['weight_max'])) {
                    if (gmp_cmp($weight, $weight_max) > 0) {
                        continue;
                    }
                }
        ...
        case 'representatives_online':
        {
            ...
            foreach ($representatives_online['representatives'] as $representative => $data) {
                if (isset($arguments['weight_min'])) {
                    if (gmp_cmp($data['weight'], $weight_min) < 0) {
                        continue;
                    }
                }
                
                if (isset($arguments['weight_max'])) {
                    if (gmp_cmp($data['weight'], $weight_max) > 0) {
                        continue;
                    }
                }
        ...
        case 'block_count':
        {
            ...
            if ($block3) {
                ...
                if (!$sync_blocks_json || !is_array($sync_blocks_array) || !isset($sync_blocks_array['count'])) {
                    $call_return['block3']['error'] = 'Failed API #1';
                    break;
                }
        ...
        case 'version':
        {
            ...
            if ($updates) {
                ...
                if (!$nano_node_json || !is_array($nano_node_array) || !isset($nano_node_array['tag_name'])) {
                    $call_return['updates']['error'] = 'Failed API #1';
                    break;
                }
        ...
        {
            if (!$C['ticker']['enable']) {
                $call_return['error'] = 'Ticker not enabled';
                break;
            }
            ...
            if (isset($arguments['amount'])) {
                ...
                $call_return['amount'] = NanoTools::RAWS['NANO'];
            }
        ...
        case 'ticker_update':
        {
            ...
            if (!$nano_vs_currency_json || !is_array($nano_vs_currencies_array) || !isset($nano_vs_currencies_array['nano'])) {
                $call_return['error'] = 'Failed API #2';
                break;
            }
        ...
        case 'tags3_update':
        {
            ...
            foreach ($third_party_tags_array as $index => $data) {
                ...
                if (array_key_exists($tag, $thirdy_party_tags_elaborated['account'])) {
                    continue;
                }
        ...
        {
            if (!$C['tags3']['enable']) {
                $call_return['error'] = 'tags3 not enabled';
                break;
            }
        ...
        case 'tag_add':
        {
            ...
            if (array_key_exists($arguments['tag'], $C['tags'][$arguments['cat']])) {
                $call_return['error'] = 'Tag already used';
                break;
            }
        ...
        case 'tag_remove':
        {
            ...
            if (!array_key_exists($arguments['tag'], $C['tags'][$arguments['cat']])) {
                $call_return['error'] = 'Tag not found';
                break;
            }
...
if ($call_return == false && $nanocall->error != null) {
    $call_return = ['error'=>$nanocall->error];
}
