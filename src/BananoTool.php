...
class BananoTool
{
    ...
    public static function den2raw($amount, string $denomination): string
    {
        if (!array_key_exists($denomination, self::RAWS)) {
            throw new BananoToolException("Invalid denomination: $denomination");
        }
    ...
    public static function den2den($amount, string $denomination_from, string $denomination_to): string
    {
        if (!array_key_exists($denomination_from, self::RAWS)) {
            throw new BananoToolException("Invalid source denomination: $denomination_from");
        }
        if (!array_key_exists($denomination_to, self::RAWS)) {
            throw new BananoToolException("Invalid target denomination: $denomination_to");
        }
    ...
    public static function account2public(string $account, bool $get_public_key = true)
    {
        if ((strpos($account, 'ban_1') === 0  ||  
            ...
            if (preg_match('/^[13456789abcdefghijkmnopqrstuwxyz]+$/', $crop)) {
                $aux = \MikeRow\Bandano\Util\Uint::fromString(substr($crop, 0, 52))->toUint4()->toArray();
                array_shift($aux);
                $key_uint4  = $aux;
                $hash_uint8 = \MikeRow\Bandano\Util\Uint::fromString(substr($crop, 52, 60))->toUint8()->toArray();
                $key_uint8  = \MikeRow\Bandano\Util\Uint::fromUint4Array($key_uint4)->toUint8();
                ...
                if (!extension_loaded('blake2')) {
                    ...
                    $key_uint8 = \MikeRow\Bandano\Util\Bin::arr2bin((array) $key_uint8);
                    $key_hash = blake2($key_uint8, 5, null, true);
                    $key_hash = \MikeRow\Bandano\Util\Bin::bin2arr(strrev($key_hash));
                }
                
                if ($hash_uint8 == $key_hash) {
                    if ($get_public_key) {
                        return \MikeRow\Bandano\Util\Uint::fromUint4Array($key_uint4)->toHexString();
                    } else {
                        return true;
                    }
                }
    ...
    public static function public2account(string $public_key): string
    {
        ...
        if (!extension_loaded('blake2')) {
            $key = \MikeRow\Bandano\Util\Uint::fromHex($public_key);
            $checksum;
            ...
            $b2b->finish($ctx, $hash);
            $hash = \MikeRow\Bandano\Util\Uint::fromUint8Array(array_slice($hash->toArray(), 0, 5))->reverse();
            $checksum = $hash->toString();
        } else {
            $key = \MikeRow\Bandano\Util\Uint::fromHex($public_key)->toUint8();
            $key = \MikeRow\Bandano\Util\Bin::arr2bin((array) $key);
            
            $hash = blake2($key, 5, null, true);
            $hash = \MikeRow\Bandano\Util\Bin::bin2arr(strrev($hash));
            $checksum = \MikeRow\Bandano\Util\Uint::fromUint8Array($hash)->toString();
        }
        
        $c_account = \MikeRow\Bandano\Util\Uint::fromHex('0' . $public_key)->toString();
    ...
    public static function private2public(string $private_key): string
    {
        ...
        $private_key = \MikeRow\Bandano\Util\Uint::fromHex($private_key)->toUint8();
        $public_key = $salt::crypto_sign_public_from_secret_key($private_key);
        
        return \MikeRow\Bandano\Util\Uint::fromUint8Array($public_key)->toHexString();
    }
    ...
    public static function string2burn(string $string, string $leading_char = '1', string $filling_char = '1'): string
    {
        ...
        if ($leading_char != '1' && $leading_char != '3') {
            throw new BananoToolException("Invalid leading character: $leading_char");
        }
        ...
        if (!extension_loaded('blake2')) {
            ...
            $key = \MikeRow\Bandano\Util\Bin::arr2bin((array) $key_uint8);
    ...
    public static function keys(bool $get_account = false): array
    {
        ...
        $keys[0] = \MikeRow\Bandano\Util\Uint::fromUint8Array(array_slice($keys[0]->toArray(), 0, 32))->toHexString();
        $keys[1] = \MikeRow\Bandano\Util\Uint::fromUint8Array($keys[1])->toHexString();
    ...
    public static function seed2keys(string $seed, int $index = 0, bool $get_account = false): array
    {
        if (strlen($seed) != 64 || !hex2bin($seed)) {
            throw new BananoToolException("Invalid seed: $seed");
        }
        if ($index < 0 || $index > 4294967295) {
            throw new BananoToolException("Invalid index: $index");
        }
        
        $seed  = \MikeRow\Bandano\Util\Uint::fromHex($seed)->toUint8();
        $index = \MikeRow\Bandano\Util\Uint::fromDec($index)->toUint8()->toArray();
        ...
        $index = \MikeRow\Bandano\Util\Uint::fromUint8Array($index)->toUint8();
        ...
        $private_key = \MikeRow\Bandano\Util\Uint::fromUint8Array(array_slice($private_key->toArray(), 0, 32))->toHexString();
    ...
    public static function mnem2hex(array $words): string
    {
        ...
        if ($mnem_count != 12 &&
            ...
            throw new BananoToolException("Invalid words array count: not 12,15,18,21,24");
        }
        ...
        foreach ($words as $index => $value) {
            ...
            if ($word === false) {
                throw new BananoToolException("Invalid menmonic word: $value");
            }
        ...
        $hex = \MikeRow\Bandano\Util\Uint::fromUint8Array($hex)->toHexString();
    ...
    public static function hex2mnem(string $hex): array
    {
        ...
        if (($hex_lenght != 32 &&
            ...
            throw new BananoToolException("Invalid hexadecimal string: $hex");
        }
        ...
        $hex = \MikeRow\Bandano\Util\Uint::fromHex($hex)->toUint8();
        $check = hash('sha256', \MikeRow\Bandano\Util\Bin::arr2bin((array) $hex), true);
        $hex  = array_merge((array) $hex, \MikeRow\Bandano\Util\Bin::bin2arr(substr($check, 0, 1)));
    ...
    {
        if (count($words) < 1) {
            throw new BananoToolException("Invalid words array count: less than 1");
        }
    ...
    public static function mseed2keys(string $mseed, int $index = 0, bool $get_account = false): array
    {
        if (strlen($mseed) != 128 || !hex2bin($mseed)) {
            throw new BananoToolException("Invalid master seed: $mseed");
        }
        if ($index < 0 || $index > 4294967295) {
            throw new BananoToolException("Invalid index: $index");
        }
    ...
    {
        if (count($hexs) < 1) {
            throw new BananoToolException("Invalid hexadecimals array count: less than 1");
        }
        if ($size < 1) {
            throw new BananoToolException("Invalid size: $size");
        }
        ...
        foreach ($hexs as $index => $value) {
            ...
            $value = \MikeRow\Bandano\Util\Uint::fromHex($value)->toUint8();
        ...
        $hash = \MikeRow\Bandano\Util\Uint::fromUint8Array($hash)->toHexString();
    ...
    {
        if (!hex2bin($msg)) {
            throw new BananoToolException("Invalid message: $msg");
        }
        if (strlen($private_key) != 64 || !hex2bin($private_key)) {
            throw new BananoToolException("Invalid private key: $private_key");
        }
        ...
        $salt = NanoSalt::instance();
        $private_key = FieldElement::fromArray(\MikeRow\Bandano\Util\Uint::fromHex($private_key)->toUint8());
        ...
        $msg = \MikeRow\Bandano\Util\Uint::fromHex($msg)->toUint8();
        ...
        return \MikeRow\Bandano\Util\Uint::fromUint8Array($signature)->toHexString();
    }
    ...
    public static function validSign(string $msg, string $sig, string $account)
    {
        ...
        if (strlen($sig) != 128 || !hex2bin($sig)) {
            throw new BananoToolException("Invalid signature: $sig");
        }
        ...
        $sig = \MikeRow\Bandano\Util\Uint::fromHex($sig)->toUint8();
        $msg = \MikeRow\Bandano\Util\Uint::fromHex($msg)->toUint8();
        $public_key  = \MikeRow\Bandano\Util\Uint::fromHex($public_key)->toUint8();
        ...
        $open2 = \MikeRow\Bandano\Util\Uint::fromUint8Array($open2)->toHexString();
    ...
    public static function mult2diff(string $difficulty, float $multiplier): string
    {
        ...
        if ($multiplier <= 0) {
            throw new BananoToolException("Invalid multiplier: $multiplier");
        }
    ...
    public static function diff2mult(string $base_difficulty, string $difficulty): float
    {
        if (strlen($base_difficulty) != 16 || !hex2bin($base_difficulty)) {
            throw new BananoToolException("Invalid base difficulty: $base_difficulty");
        }
        if (strlen($difficulty) != 16 || !hex2bin($difficulty)) {
            throw new BananoToolException("Invalid difficulty: $difficulty");
        }
    ...
    public static function work(string $hash, string $difficulty): string
    {
        if (strlen($hash) != 64 || !hex2bin($hash)) {
            throw new BananoToolException("Invalid hash: $hash");
        }
        if (strlen($difficulty) != 16 || !hex2bin($difficulty)) {
            throw new BananoToolException("Invalid difficulty: $difficulty");
        }
        
        $hash = \MikeRow\Bandano\Util\Uint::fromHex($hash)->toUint8();
        $difficulty = hex2bin($difficulty);
        
        if (!extension_loaded('blake2')) {
            $b2b = new Blake2b();
            $rng = random_bytes(8);
            $rng = \MikeRow\Bandano\Util\bin2arr($rng);
            ...
            while (true) {
                ...
                //$output = \MikeRow\Bandano\Util\Uint::fromUint8Array($output)->toHexString();
                
                if (strcasecmp(\MikeRow\Bandano\Util\Bin::arr2bin($output), $difficulty) >= 0) {
                    return \MikeRow\Bandano\Util\Uint::fromUint8Array(array_reverse($rng))->toHexString();
                }

                $rng = $output;
            }
        } else {
            $hash = \MikeRow\Bandano\Util\Bin::arr2bin((array) $hash);
            $rng = random_bytes(8);
            ...
            while (true) {
                ...
                if (strcasecmp($output, $difficulty) >= 0) {
                    return \MikeRow\Bandano\Util\Uint::fromUint8Array(array_reverse(\MikeRow\Bandano\Util\bin2arr($rng)))->toHexString();
                }
    ...
    public static function validWork(string $hash, string $difficulty, string $work): bool
    {
        ...
        $hash = \MikeRow\Bandano\Util\Uint::fromHex($hash)->toUint8();
        $work = \MikeRow\Bandano\Util\Uint::fromHex($work)->toUint8();
        ...
        $res = \MikeRow\Bandano\Util\Uint::fromUint8Array($res)->toHexString();
